...
export default class HistoricoService {
  static async registrar(dados: any) {
    ...
    return await HistoricoModel.criar(dados);
  }
  ...
  static async listarPorOcorrencia(id_ocorrencia: number) {
    ...
    return await HistoricoModel.listarPorOcorrencia(id_ocorrencia);
  }
  ...
  static async listarPorUsuario(id_usuario: number) {
    ...
    return await HistoricoModel.listarPorUsuario(id_usuario);
  }

  static async buscarPorId(id_historico: number) {
    const historico = await HistoricoModel.buscarPorId(id_historico);
    if (!historico) {
