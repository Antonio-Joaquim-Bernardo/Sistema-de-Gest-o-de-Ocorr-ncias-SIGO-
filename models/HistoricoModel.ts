...
export default class HistoricoModel {
  ...
  static async listarTodos() {
    const sql = `
      SELECT h.id_historico, h.acao, h.data_registro, h.id_ocorrencia, h.id_usuario,
             u.nome_completo, u.tipo_funcao, o.titulo
      FROM historico h
      INNER JOIN usuarios u ON h.id_usuario = u.id_usuario
      INNER JOIN ocorrencias o ON h.id_ocorrencia = o.id_ocorrencia
  ...
  static async listarPorUsuario(id_usuario: number) {
    const sql = `
      SELECT h.id_historico, h.acao, h.data_registro, h.id_ocorrencia, o.titulo
      FROM historico h
      INNER JOIN ocorrencias o ON h.id_ocorrencia = o.id_ocorrencia
  ...
  static async buscarPorId(id_historico: number) {
    const sql = `SELECT * FROM historico WHERE id_historico = $1;`;
    const resultado = await pool.query(sql, [id_historico]);
